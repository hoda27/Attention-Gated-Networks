...
class UKBBDataset(data.Dataset):
    def __init__(self, root_dir, split, transform=None, preload_data=False):
        ...
        if self.preload_data:
            ...
            self.raw_labels = [load_nifti_img(ii, dtype=np.float32)[0] for ii in self.target_filenames]
    ...
    def __getitem__(self, index):
        ...
        if not self.preload_data:
            ...
            target, _ = load_nifti_img(self.target_filenames[index], dtype=np.float32)
        else:
